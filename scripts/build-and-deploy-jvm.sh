...
readonly IMAGE="$REGISTRY/${PROJECT_NAME}-jvm:$PROJECT_VERSION-$(date +%Y%m%d%H%M%S)"
...
deploy() {
  kubectl create deploy "${PROJECT_NAME}-jvm" --image $PROJECT_NAME -o json --dry-run=client | jq ".spec.template.spec.containers[0].image=\"$IMAGE\"" | kubectl apply -f -
}
